 ...
 */
const getIdPEndpoints = async (idpurl) => {
  if (!idpurl) {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch IdP endpoints from ${idpurl}: ${response.status}: ${response.statusText}`);
  }

  const data = await response.json();
  if (!data.authorization_endpoint || !data.token_endpoint) {
    throw new Error(`Invalid IdP configuration from ${idpurl}: missing authorization or token endpoint`);
  }
 ...
 */
const getAuthEndpoint = async (idpurl) => {
  const idpData = await getIdPEndpoints(idpurl);
  return idpData.authorization_endpoint;
};
 ...
 */
const getTokenEndpoint = async (idpurl) => {
  const idpData = await getIdPEndpoints(idpurl);
  return idpData.token_endpoint;
};
...
export const login = async (idpurl, idpcallback, idpappid) => {
  if (!idpurl || !idpcallback || !idpappid) {
    FauxtonAPI.addNotification({
      msg: 'Missing IdP URL, callback or app ID',
      type: 'error'
    });
    throw new Error('Missing IdP URL, callback or app ID');
  }

  const authEndpoint = await getAuthEndpoint(idpurl);
...
export const codeToToken = async (url) => {
  ...
  if (authCode) {
    ...
    const authUrl = await getTokenEndpoint(idpurl);
    const fetchOptions = {
      method: 'POST',
      headers: {
